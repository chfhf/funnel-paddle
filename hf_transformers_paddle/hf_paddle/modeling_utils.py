...
from paddorch import Tensor, device, nn
from paddorch.nn import CrossEntropyLoss
...
try:
    from paddorch.nn import Identity
except ImportError:
...
def get_parameter_dtype(parameter: Union[nn.Module, GenerationMixin, "ModuleUtilsMixin"]):
    try:
        return  parameter.parameters()[0].dtype
    except StopIteration:
        # For nn.DataParallel compatibility in PyTorch 1.5
...
class ModuleUtilsMixin:
    ...
    def reset_memory_hooks_state(self):
        ...
        :func:`~hf_paddle.modeling_utils.ModuleUtilsMixin.add_memory_hooks`).
        """
        for module in self.modules():
    ...
    def invert_attention_mask(self, encoder_attention_mask: Tensor) -> Tensor:
        ...
        if encoder_attention_mask.dim() == 3:
            encoder_extended_attention_mask = encoder_attention_mask.unsqueeze(1) #[:, None, :, :]
        if encoder_attention_mask.dim() == 2:
            encoder_extended_attention_mask = encoder_attention_mask.unsqueeze(1).unsqueeze(1)  #[:, None, None, :]
        # T5 has a mask that can compare sequence ids, we can simulate this here with this transposition
        # Cf. https://github.com/tensorflow/mesh/blob/8d2465e9bc93129b913b5ccc6a59aa97abd96ec6/mesh_tensorflow
        # /transformer/transformer_layers.py#L270
        # encoder_extended_attention_mask = (encoder_extended_attention_mask ==
        # encoder_extended_attention_mask,True)
    ...
    def get_extended_attention_mask(self, attention_mask: Tensor, input_shape: Tuple[int], device: device) -> Tensor:
        ...
        if attention_mask.dim() == 3:
            extended_attention_mask = attention_mask.unsqueeze(1) #[:, None, :, :]
            ...
            if self.config.is_decoder:
                ...
                causal_mask = seq_ids.unsqueeze(0).unsqueeze(0).repeat(batch_size, seq_length, 1) <= seq_ids.unsqueeze(0).unsqueeze(-1)#[None, :, None]
                ...
                extended_attention_mask = causal_mask.unsqueeze(1)* attention_mask.unsqueeze(1).unsqueeze(1) # [:, None, :, :] * attention_mask[:, None, None, :]
            else:
                extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(1)#[:, None, None, :]
        else:
            raise ValueError(
    ...
    def floating_point_ops(
        ...
        <https://arxiv.org/pdf/2001.08361.pdf>`__ section 2.1. Should be overridden for hf_paddle with parameter
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    :class:`~hf_paddle.PreTrainedModel` takes care of storing the configuration of the models and handles methods
    ...
    Class attributes (overridden by derived classes):
        ...
        - **config_class** (:class:`~hf_paddle.PretrainedConfig`) -- A subclass of
          :class:`~hf_paddle.PretrainedConfig` to use as configuration class for this model architecture.
        - **load_tf_weights** (:obj:`Callable`) -- A python `method` for loading a TensorFlow checkpoint in a PyTorch
          model, taking as arguments:

            - **model** (:class:`~hf_paddle.PreTrainedModel`) -- An instance of the model on which to load the
              TensorFlow checkpoint.
            - **config** (:class:`~hf_paddle.PreTrainedConfig`) -- An instance of the configuration associated to
              the model.
    ...
    def _get_resized_lm_head(
        ...
        Args:
            ...
            transposed (:obj:`bool`, `optional`, defaults to :obj:`False`):
                Whether ``old_lm_head`` is transposed or not. If True ``old_lm_head.shape`` is ``lm_head_dim,
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        `:func:`~hf_paddle.PreTrainedModel.from_pretrained`` class method.
        ...
        dtype = "torch.float32"#get_parameter_dtype(model_to_save)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        Parameters:
            pretrained_model_name_or_path (:obj:`str` or :obj:`os.PathLike`, `optional`):
                Can be either:
                    ...
                    - A path to a `directory` containing model weights saved using
                      :func:`~hf_paddle.PreTrainedModel.save_pretrained`, e.g., ``./my_model_directory/``.
            ...
            config (:obj:`Union[PretrainedConfig, str, os.PathLike]`, `optional`):
                Can be either:
                    ...
                    - an instance of a class derived from :class:`~hf_paddle.PretrainedConfig`,
                    - a string or path valid as input to :func:`~hf_paddle.PretrainedConfig.from_pretrained`.
                ...
                be automatically loaded when:
                    ...
                    - The model is a model provided by the library (loaded with the `model id` string of a pretrained
                      model).
                    - The model was saved using :func:`~hf_paddle.PreTrainedModel.save_pretrained` and is reloaded
            ...
            state_dict (:obj:`Dict[str, torch.Tensor]`, `optional`):
                ...
                :func:`~hf_paddle.PreTrainedModel.save_pretrained` and
                :func:`~hf_paddle.PreTrainedModel.from_pretrained` is not a simpler option.
            ...
            torch_dtype (:obj:`str` or :obj:`torch.dtype`, `optional`):
                ...
                .. warning::
                    ...
                    ``hf_paddle.__version__ < 4.6.0`` for seeded model initialization. This argument will be removed
                    at the next major version. See `pull request 11471
                    <https://github.com/huggingface/hf_paddle/pull/11471>`__ for more information.
            ...
            kwargs (remaining dictionary of keyword arguments, `optional`):
                ...
                automatically loaded:
                    ...
                    - If a configuration is not provided, ``kwargs`` will be first passed to the configuration class
                      initialization function (:func:`~hf_paddle.PretrainedConfig.from_pretrained`). Each key of
        ...
        .. note::
            ...
            <https://huggingface.co/hf_paddle/installation.html#offline-mode>`__ to use this method in a firewalled
            environment.

        Examples::

            >>> from hf_paddle import BertConfig, BertModel
        ...
        if from_pt:
            if state_dict is None:
                try:
                    if  resolved_archive_file.endswith(".pt") or resolved_archive_file.endswith(".bin"):
                        pt_file=resolved_archive_file
                        pd_file=resolved_archive_file+".pdparams"
                        resolved_archive_file = pd_file
                        if   not os.path.isfile(pd_file):
                            ###resave use paddle save
                            import torch as pytorch
                            from paddorch.convert_pretrain_model import load_pytorch_pretrain_model_remove_prefix

                            pytorch_state_dict=pytorch.load(pt_file, map_location="cpu")

                            with no_init_weights(_enable=_fast_init):
                                paddle_model = cls(config, *model_args, **model_kwargs)
                            paddle_state_dict=load_pytorch_pretrain_model_remove_prefix(paddle_model, pytorch_state_dict,pytorch_prefix=paddle_model.base_model_prefix+".")
                            torch.save(paddle_state_dict,pd_file)
                            print("save paddle state dict:",pd_file)


                    state_dict = torch.load(resolved_archive_file, map_location="cpu")
                    print("loaded model file:",resolved_archive_file)
                except Exception:
                    raise OSError(
    ...
    def _load_state_dict_into_model(
        ...
        for key in state_dict.keys():
            ...
            if prefix+"."+key in expected_keys:
                new_key=prefix+"."+key
        ...
        #         missing_keys, add_prefix="", remove_prefix=""
        #     )
...
class PoolerStartLogits(nn.Module):
    ...
    Args:
        config (:class:`~hf_paddle.PretrainedConfig`):
...
class PoolerEndLogits(nn.Module):
    ...
    def forward(
        ...
        if start_positions is not None:
            ...
            start_positions = start_positions.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, hsz)
...
class PoolerAnswerClass(nn.Module):
    ...
    def forward(
        ...
        if start_positions is not None:
            start_positions = start_positions.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, hsz) #[:, None, None].expand(-1, -1, hsz)  # shape (bsz, 1, hsz)
            start_states = hidden_states.gather(-2, start_positions).squeeze(-2)  # shape (bsz, hsz)

        if cls_index is not None:
            cls_index = cls_index.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, hsz) #[:, None, None].expand(-1, -1, hsz)  # shape (bsz, 1, hsz)
...
class SquadHeadOutput(ModelOutput):
    """
    Base class for outputs of question answering models using a :class:`~hf_paddle.modeling_utils.SQuADHead`.
    ...
    def forward(
        ...
        Args:
            ...
            return_dict (:obj:`bool`, `optional`, defaults to :obj:`False`):
                Whether or not to return a :class:`~hf_paddle.file_utils.ModelOutput` instead of a plain tuple.
...
def prune_conv1d_layer(layer: Conv1D, index: torch.LongTensor, dim: int = 1) -> Conv1D:
    ...
    Args:
        layer (:class:`~hf_paddle.modeling_utils.Conv1D`): The layer to prune.
        index (:obj:`torch.LongTensor`): The indices to keep in the layer.
        dim (:obj:`int`, `optional`, defaults to 1): The dimension on which to keep the indices.

    Returns:
        :class:`~hf_paddle.modeling_utils.Conv1D`: The pruned layer as a new layer with :obj:`requires_grad=True`.
    """
    index = index.to(layer.weight.device)
    W = layer.weight.index_select(dim, index).clone().detach()
    if dim == 0:
...
def prune_layer(
    ...
    Returns:
        :obj:`torch.nn.Linear` or :class:`~hf_paddle.modeling_utils.Conv1D`: The pruned layer as a new layer with
