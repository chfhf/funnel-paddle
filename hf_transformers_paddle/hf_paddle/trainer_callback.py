...
class TrainerState:
    """
    A class containing the :class:`~hf_paddle.Trainer` inner state that will be saved along the model and optimizer
    when checkpointing and passed to the :class:`~hf_paddle.TrainerCallback`.
...
class TrainerControl:
    """
    A class that handles the :class:`~hf_paddle.Trainer` control flow. This class is used by the
    :class:`~hf_paddle.TrainerCallback` to activate some switches in the training loop.
...
class TrainerCallback:
    ...
    Args:
        args (:class:`~hf_paddle.TrainingArguments`):
            The training arguments used to instantiate the :class:`~hf_paddle.Trainer`.
        state (:class:`~hf_paddle.TrainerState`):
            The current state of the :class:`~hf_paddle.Trainer`.
        control (:class:`~hf_paddle.TrainerControl`):
            The object that is returned to the :class:`~hf_paddle.Trainer` and can be used to make some decisions.
        model (:class:`~hf_paddle.PreTrainedModel` or :obj:`torch.nn.Module`):
            The model being trained.
        tokenizer (:class:`~hf_paddle.PreTrainedTokenizer`):
    ...
    def on_init_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        """
        Event called at the end of the initialization of the :class:`~hf_paddle.Trainer`.
        """
        pass
...
class DefaultFlowCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that handles the default flow of the training loop for logs, evaluation
    and checkpoints.
    """
...
class ProgressCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that displays the progress of training or evaluation.
    """
...
class PrinterCallback(TrainerCallback):
    """
    A bare :class:`~hf_paddle.TrainerCallback` that just prints the logs.
    """
...
class EarlyStoppingCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that handles early stopping.
    ...
    This callback depends on :class:`~hf_paddle.TrainingArguments` argument `load_best_model_at_end` functionality
    to set best_metric in :class:`~hf_paddle.TrainerState`.
    """
