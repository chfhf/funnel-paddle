...
def run_hp_search_ray(trainer, n_trials: int, direction: str, **kwargs) -> BestRun:
    ...
    def _objective(trial, local_trainer, checkpoint_dir=None):
        try:
            from hf_paddle.utils.notebook import NotebookProgressCallback
    ...
    def dynamic_modules_import_trainable(*args, **kwargs):
        ...
        Without this, an ImportError will be thrown. See https://github.com/huggingface/hf_paddle/issues/11565.
...
class TensorBoardCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that sends the logs to `TensorBoard
    ...
    def __init__(self, tb_writer=None):
        ...
        if has_tensorboard:
            try:
                from paddorch.utils.tensorboard import SummaryWriter  # noqa: F401
...
class WandbCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that sends the logs to `Weight and Biases <https://www.wandb.com/>`__.
    """
...
class CometCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that sends the logs to `Comet ML <https://www.comet.ml/site/>`__.
    """
    ...
    def setup(self, args, state, model):
        ...
        if state.is_world_process_zero:
            ...
            if experiment is not None:
                experiment._set_model_graph(model, framework="hf_paddle")
                experiment._log_parameters(args, prefix="args/", framework="hf_paddle")
                if hasattr(model, "config"):
                    experiment._log_parameters(model.config, prefix="config/", framework="hf_paddle")
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            if experiment is not None:
                experiment._log_metrics(logs, step=state.global_step, epoch=state.epoch, framework="hf_paddle")


class AzureMLCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that sends the logs to `AzureML
...
class MLflowCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that sends the logs to `MLflow <https://www.mlflow.org/>`__.
    """
    ...
    def setup(self, args, state, model):
        ...
        Environment:
            HF_MLFLOW_LOG_ARTIFACTS (:obj:`str`, `optional`):
                ...
                whatever is in :class:`~hf_paddle.TrainingArguments`'s ``output_dir`` to the local or remote
...
class NeptuneCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that sends the logs to `Neptune <https://neptune.ai>`.
    """
...
class CodeCarbonCallback(TrainerCallback):
    """
    A :class:`~hf_paddle.TrainerCallback` that tracks the CO2 emission of training.
    """
