...
from hf_paddle.utils.versions import importlib_metadata
...
if USE_TORCH in ENV_VARS_TRUE_AND_AUTO_VALUES and USE_TF not in ENV_VARS_TRUE_VALUES:
    _torch_available = importlib.util.find_spec("paddorch") is not None
    # if _torch_available:
    #     try:
    #         _torch_version = importlib_metadata.version("paddle")
    #         logger.info(f"Paddle version {_torch_version} available.")
...
old_default_cache_path = os.path.join(torch_cache_home, "hf_paddle")
# New default cache, shared with the Datasets library
hf_cache_home = os.path.expanduser(
    os.getenv("HF_HOME", os.path.join(os.getenv("XDG_CACHE_HOME", "~/.cache"), "huggingface"))
)
default_cache_path = os.path.join(hf_cache_home, "hf_paddle")
...
if (
    ...
    logger.warning(
        ...
        "'~/.cache/torch/hf_paddle' to '~/.cache/huggingface/hf_paddle'. Since you don't seem to have overridden "
        "and '~/.cache/torch/hf_paddle' is a directory that exists, we're moving it to "
        "'~/.cache/huggingface/hf_paddle' to avoid redownloading models you have already in the cache. You should "
...
#    )
    ...
    _torch_onnx_dict_inputs_support_available = False #torch_version >= TORCH_ONNX_DICT_INPUTS_MINIMUM_VERSION
...
def add_start_docstrings_to_model_forward(*docstr):
    def docstring_decorator(fn):
        class_name = f":class:`~hf_paddle.{fn.__qualname__.split('.')[0]}`"
...
PT_RETURN_INTRODUCTION = r"""
    Returns:
        ...
        various elements depending on the configuration (:class:`~hf_paddle.{config_class}`) and inputs.
...
TF_RETURN_INTRODUCTION = r"""
    Returns:
        ...
        elements depending on the configuration (:class:`~hf_paddle.{config_class}`) and inputs.
...
PT_TOKEN_CLASSIFICATION_SAMPLE = r"""
    Example::
        ...
        >>> from hf_paddle import {tokenizer_class}, {model_class}
        >>> import paddorch as torch
        ...
        >>> labels = torch.tensor([1] * inputs["input_ids"].shape[1]).unsqueeze(0)  # Batch size 1
...
def hf_bucket_url(
    ...
    its sha1 if stored in git, or its sha256 if stored in git-lfs. Files cached locally from hf_paddle before v3.5.0
...
def filename_to_url(filename, cache_dir=None):
    ...
    file_ext=comps[-1]
    if len(file_ext)<5:
        filename= ".".join(comps[:-1]) ##remove the lsat extension
...
def get_cached_models(cache_dir: Union[str, Path] = None) -> List[Tuple]:
    ...
    Args:
        cache_dir (:obj:`Union[str, Path]`, `optional`):
            The cache directory to search for models within. Will default to the hf_paddle cache if unset.
...
def cached_path(
    ...
    if is_remote_url(url_or_filename):
        url_or_filename=url_or_filename.replace("model_state.pdparams","pytorch_model.bin")
        url_or_filename = url_or_filename.replace("model_config.json", "config.json")
...
def http_user_agent(user_agent: Union[Dict, str, None] = None) -> str:
    ...
    """
    ua = f"hf_paddle/{__version__}; python/{sys.version.split()[0]}; session_id/{SESSION_ID}"
    if is_torch_available():
        ua += f"; paddorch"
    if is_tf_available():
...
class ModelOutput(OrderedDict):
    ...
    .. warning::
        You can't unpack a :obj:`ModelOutput` directly. Use the :meth:`~hf_paddle.file_utils.ModelOutput.to_tuple`
...
class PushToHubMixin:
    ...
    def push_to_hub(
        ...
        Parameters:
            ...
            use_auth_token (:obj:`bool` or :obj:`str`, `optional`):
                The token to use as HTTP bearer authorization for remote files. If :obj:`True`, will use the token
                generated when running :obj:`hf_paddle-cli login` (stored in :obj:`~/.huggingface`). Will default to
        ...
        Examples::
            ...
            from hf_paddle import {object_class}
    ...
    def _get_repo_url_from_name(
        ...
        if isinstance(use_auth_token, str):
            ...
            if token is None:
                raise ValueError(
                    "You must login to the Hugging Face hub on this computer by typing `hf_paddle-cli login` and "
