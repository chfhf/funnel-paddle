...
PROCESS_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size * num_beams, sequence_length)`):
            ...
            Indices can be obtained using any class inheriting from :class:`~hf_paddle.PreTrainedTokenizer`. See
...
class BeamScorer(ABC):
    """
    Abstract base class for all beam scorers that are used for :meth:`~hf_paddle.PreTrainedModel.beam_search` and
    :meth:`~hf_paddle.PreTrainedModel.beam_sample`.
    """
...
class BeamSearchScorer(BeamScorer):
    r"""
    :class:`hf_paddle.BeamScorer` implementing standard beam search decoding.
    ...
    def process(
        ...
        import paddle
        if isinstance(self._done,paddle.Tensor):
            self._done=self._done.numpy()
    ...
    def finalize(
        ...
        decoded = input_ids.new(batch_size * self.num_beam_hyps_to_keep, sent_max_len).long()
        ...
        for i, hypo in enumerate(best):
            decoded[i, : int(sent_lengths[i]) ] = hypo
            if sent_lengths[i] < max_length:
                decoded[i, int(sent_lengths[i])] = eos_token_id
        return UserDict(
            {
                "sequences": decoded,
