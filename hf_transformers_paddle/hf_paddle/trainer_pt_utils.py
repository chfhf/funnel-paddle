...
from paddorch.utils.data.dataset import Dataset, IterableDataset
from paddorch.utils.data.distributed import DistributedSampler
from paddorch.utils.data.sampler import RandomSampler, Sampler
...
try:
    from paddorch.optim.lr_scheduler import SAVE_STATE_WARNING
except ImportError:
    SAVE_STATE_WARNING = ""
...
class IterableDatasetShard(IterableDataset):
    ...
    .. warning:
        ...
        :meth:`~hf_paddle.trainer_pt_utils.IterableDatasetShard.set_epoch` method of this object. It will set the
    ...
    Args:
        ...
        seed (:obj:`int`, `optional`, defaults to 0):
            ...
            :meth:`~hf_paddle.trainer_pt_utils.IterableDatasetShard.set_epoch`.
    """
...
def log_metrics(self, split, metrics):
    ...
    **Understanding the reports:**
    ...
    Note that this tracker doesn't account for memory allocations outside of :class:`~hf_paddle.Trainer`'s
    ...
    This also means that if any other tool that is used along the :class:`~hf_paddle.Trainer` calls
    ``torch.cuda.reset_peak_memory_stats``, the gpu peak memory stats could be invalid. And the
    :class:`~hf_paddle.Trainer` will disrupt the normal behavior of any such tools that rely on calling
...
def save_metrics(self, split, metrics, combined=True):
    ...
    To understand the metrics please read the docstring of :meth:`~hf_paddle.Trainer.log_metrics`. The only
