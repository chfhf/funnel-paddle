...
import paddorch.distributed as dist
...
class GenerationMixin:
    ...
    :class:`~hf_paddle.PreTrainedModel`.
    """

    def prepare_inputs_for_generation(self, input_ids: torch.LongTensor, **kwargs) -> Dict[str, Any]:
        """
        Implement in subclasses of :class:`~hf_paddle.PreTrainedModel` for custom behavior to prepare inputs in the
        generate method.
        """
    ...
    def adjust_logits_during_generation(self, logits: torch.FloatTensor, **kwargs) -> torch.FloatTensor:
        """
        Implement in subclasses of :class:`~hf_paddle.PreTrainedModel` for custom behavior to adjust the logits in
        the generate method.
        """
        return logits
    ...
    def _get_logits_warper(
        ...
        """
        This class returns a :obj:`~hf_paddle.LogitsProcessorList` list object that contains all relevant
        :obj:`~hf_paddle.LogitsWarper` instances used for multinomial sampling.
        """
        ...
        # the following idea is largely copied from this PR: https://github.com/huggingface/hf_paddle/pull/5420/files
    ...
    def _get_logits_processor(
        ...
        """
        This class returns a :obj:`~hf_paddle.LogitsProcessorList` list object that contains all relevant
        :obj:`~hf_paddle.LogitsProcessor` instances used to modify the scores of the language model head.
        """
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        attribute of the same name inside the :class:`~hf_paddle.PretrainedConfig` of the model. The default values
        ...
        Return:
            :class:`~hf_paddle.file_utils.ModelOutput` or :obj:`torch.LongTensor`: A
            :class:`~hf_paddle.file_utils.ModelOutput` (if ``return_dict_in_generate=True`` or when
            ``config.return_dict_in_generate=True``) or a :obj:`torch.FloatTensor`.

                If the model is `not` an encoder-decoder model (``model.config.is_encoder_decoder=False``), the
                possible :class:`~hf_paddle.file_utils.ModelOutput` types are:

                    - :class:`~hf_paddle.generation_utils.GreedySearchDecoderOnlyOutput`,
                    - :class:`~hf_paddle.generation_utils.SampleDecoderOnlyOutput`,
                    - :class:`~hf_paddle.generation_utils.BeamSearchDecoderOnlyOutput`,
                    - :class:`~hf_paddle.generation_utils.BeamSampleDecoderOnlyOutput`

                If the model is an encoder-decoder model (``model.config.is_encoder_decoder=True``), the possible
                :class:`~hf_paddle.file_utils.ModelOutput` types are:

                    - :class:`~hf_paddle.generation_utils.GreedySearchEncoderDecoderOutput`,
                    - :class:`~hf_paddle.generation_utils.SampleEncoderDecoderOutput`,
                    - :class:`~hf_paddle.generation_utils.BeamSearchEncoderDecoderOutput`,
                    - :class:`~hf_paddle.generation_utils.BeamSampleEncoderDecoderOutput`

        Examples::
            >>> from hf_paddle import AutoTokenizer, AutoModelForCausalLM, AutoModelForSeq2SeqLM
        ...
        **model_kwargs,
        ...
        Parameters:
            ...
            logits_processor (:obj:`LogitsProcessorList`, `optional`):
                An instance of :class:`~hf_paddle.LogitsProcessorList`. List of instances of class derived from
                :class:`~hf_paddle.LogitsProcessor` used to modify the prediction scores of the language modeling
                head applied at each generation step.
            stopping_criteria (:obj:`StoppingCriteriaList`, `optional`):
                An instance of :class:`~hf_paddle.StoppingCriteriaList`. List of instances of class derived from
                :class:`~hf_paddle.StoppingCriteria` used to tell if the generation loop should stop.
        ...
        Return:
            :class:`~hf_paddle.generation_utils.GreedySearchDecoderOnlyOutput`,
            :class:`~hf_paddle.generation_utils.GreedySearchEncoderDecoderOutput` or obj:`torch.LongTensor`: A
            :obj:`torch.LongTensor` containing the generated tokens (default behaviour) or a
            :class:`~hf_paddle.generation_utils.GreedySearchDecoderOnlyOutput` if
            ``model.config.is_encoder_decoder=False`` and ``return_dict_in_generate=True`` or a
            :class:`~hf_paddle.generation_utils.GreedySearchEncoderDecoderOutput` if
            ``model.config.is_encoder_decoder=True``.

        Examples::

            >>> from hf_paddle import (
            ... AutoTokenizer,
        ...
        **model_kwargs,
        ...
        Parameters:
            ...
            logits_warper (:obj:`LogitsProcessorList`, `optional`):
                An instance of :class:`~hf_paddle.LogitsProcessorList`. List of instances of class derived from
                :class:`~hf_paddle.LogitsWarper` used to warp the prediction score distribution of the language
        ...
        Return:
            :class:`~hf_paddle.generation_utils.SampleDecoderOnlyOutput`,
            :class:`~hf_paddle.generation_utils.SampleEncoderDecoderOutput` or obj:`torch.LongTensor`: A
            :obj:`torch.LongTensor` containing the generated tokens (default behaviour) or a
            :class:`~hf_paddle.generation_utils.SampleDecoderOnlyOutput` if
            ``model.config.is_encoder_decoder=False`` and ``return_dict_in_generate=True`` or a
            :class:`~hf_paddle.generation_utils.SampleEncoderDecoderOutput` if
            ``model.config.is_encoder_decoder=True``.
        ...
        **model_kwargs,
        ...
        Parameters:
            ...
            beam_scorer (:obj:`BeamScorer`):
                An derived instance of :class:`~hf_paddle.BeamScorer` that defines how beam hypotheses are
                constructed, stored and sorted during generation. For more information, the documentation of
                :class:`~hf_paddle.BeamScorer` should be read.
            logits_processor (:obj:`LogitsProcessorList`, `optional`):
                An instance of :class:`~hf_paddle.LogitsProcessorList`. List of instances of class derived from
                :class:`~hf_paddle.LogitsProcessor` used to modify the prediction scores of the language modeling
                head applied at each generation step.
        ...
        Return:
            :class:`~hf_paddle.generation_utilsBeamSearchDecoderOnlyOutput`,
            :class:`~hf_paddle.generation_utils.BeamSearchEncoderDecoderOutput` or obj:`torch.LongTensor`: A
            :obj:`torch.LongTensor` containing the generated tokens (default behaviour) or a
            :class:`~hf_paddle.generation_utils.BeamSearchDecoderOnlyOutput` if
            ``model.config.is_encoder_decoder=False`` and ``return_dict_in_generate=True`` or a
            :class:`~hf_paddle.generation_utils.BeamSearchEncoderDecoderOutput` if
            ``model.config.is_encoder_decoder=True``.
        ...
        while True:
            ...
            input_ids = torch.cat([torch.index_select(input_ids, index=beam_idx,dim=0), beam_next_tokens.unsqueeze(-1)], dim=-1)
        ...
        **model_kwargs,
        ...
        Parameters:
            ...
            input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
                ...
                :obj:`torch.LongTensor` of shape :obj:`(1,)`.
            beam_scorer (:obj:`BeamScorer`):
                A derived instance of :class:`~hf_paddle.BeamScorer` that defines how beam hypotheses are
                constructed, stored and sorted during generation. For more information, the documentation of
                :class:`~hf_paddle.BeamScorer` should be read.
            logits_processor (:obj:`LogitsProcessorList`, `optional`):
                An instance of :class:`~hf_paddle.LogitsProcessorList`. List of instances of class derived from
        ...
        Return:
            :class:`~hf_paddle.generation_utils.BeamSampleDecoderOnlyOutput`,
            :class:`~hf_paddle.generation_utils.BeamSampleEncoderDecoderOutput` or obj:`torch.LongTensor`: A
            :obj:`torch.LongTensor` containing the generated tokens (default behaviour) or a
            :class:`~hf_paddle.generation_utils.BeamSampleDecoderOnlyOutput` if
            ``model.config.is_encoder_decoder=False`` and ``return_dict_in_generate=True`` or a
            :class:`~hf_paddle.generation_utils.BeamSampleEncoderDecoderOutput` if
            ``model.config.is_encoder_decoder=True``.
