...
path = /path/to/hf_paddle/.env/bin/python

args = -m debugpy --listen 5678 --wait-for-client /path/to/hf_paddle/src/hf_paddle/commands/hf_paddle_cli.py
...
class LfsCommands(BaseTransformersCLICommand):
    ...
    1. $ hf_paddle-cli lfs-enable-largefiles
    ...
    2. $ hf_paddle-cli lfs-multipart-upload
...
class LfsEnableCommand:
    ...
    def run(self):
        warnings.warn(
            "Managing repositories through hf_paddle-cli is deprecated. Please use `huggingface-cli` instead."
        )
        ...
        if not os.path.isdir(local_path):
            ...
            exit(1)
        subprocess.run(
            "git config lfs.customtransfer.multipart.path hf_paddle-cli".split(), check=True, cwd=local_path
        )
        subprocess.run(
