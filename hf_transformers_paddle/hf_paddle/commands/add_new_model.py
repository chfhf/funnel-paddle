    ...
    def run(self):
        if not _has_cookiecutter:
            raise ImportError(
                ...
                "the following at the root of your `hf_paddle` clone:\n\n\t$ pip install -e .[modelcreation]\n"
            )
        ...
        model_dir = f"{path_to_transformer_root}/src/hf_paddle/models/{lowercase_model_name}"
        ...
        def remove_copy_lines(path):
            ...
            with open(path, "w") as f:
                for line in lines:
                    if "# Copied from hf_paddle." not in line:
                        f.write(line)
