...
class LogitsProcessorList(list):
    """
    This class can be used to create a list of :class:`~hf_paddle.LogitsProcessor` or
    :class:`~hf_paddle.LogitsWarper` to subsequently process a :obj:`scores` input tensor. This class inherits from
    list and adds a specific `__call__` method to apply each :class:`~hf_paddle.LogitsProcessor` or
    :class:`~hf_paddle.LogitsWarper` to the inputs.
    """
...
class MinLengthLogitsProcessor(LogitsProcessor):
    r"""
    :class:`hf_paddle.LogitsProcessor` enforcing a min-length by setting EOS probability to 0.
...
class TemperatureLogitsWarper(LogitsWarper):
    r"""
    :class:`hf_paddle.LogitsWarper` for temperature (exponential scaling output probability distribution).
...
class RepetitionPenaltyLogitsProcessor(LogitsProcessor):
    r"""
    :class:`hf_paddle.LogitsProcessor` enforcing an exponential penalty on repeated sequences.
...
class TopPLogitsWarper(LogitsWarper):
    """
    :class:`hf_paddle.LogitsWarper` that performs top-p, i.e. restricting to top tokens summing to prob_cut_off <=
    prob_cut_off.
...
class TopKLogitsWarper(LogitsWarper):
    r"""
    :class:`hf_paddle.LogitsWarper` that performs top-k, i.e. restricting to the k highest probability elements.
...
class NoRepeatNGramLogitsProcessor(LogitsProcessor):
    r"""
    :class:`hf_paddle.LogitsProcessor` that enforces no repetition of n-grams. See `Fairseq
...
class EncoderNoRepeatNGramLogitsProcessor(LogitsProcessor):
    r"""
    :class:`hf_paddle.LogitsProcessor` that enforces no repetition of encoder input ids n-grams for the decoder ids.
...
class NoBadWordsLogitsProcessor(LogitsProcessor):
    """
    :class:`hf_paddle.LogitsProcessor` that enforces that specified sequences will never be sampled.
    ...
    def _set_scores_to_inf_for_banned_tokens(self, scores: torch.Tensor, banned_tokens: List[List[int]]) -> None:
        ...
        banned_mask = (
            torch.sparse.LongTensor(banned_mask.t(), indices, scores.shape).to(scores.device).to_dense().bool()
        )
...
class PrefixConstrainedLogitsProcessor(LogitsProcessor):
    r"""
    :class:`hf_paddle.LogitsProcessor` that enforces constrained generation and is useful for prefix-conditioned
...
class HammingDiversityLogitsProcessor(LogitsProcessor):
    r"""
    :class:`hf_paddle.LogitsProcessor` that enforces diverse beam search. Note that this logits processor is only
    effective for :meth:`hf_paddle.PreTrainedModel.group_beam_search`. See `Diverse Beam Search: Decoding Diverse
...
class ForcedBOSTokenLogitsProcessor(LogitsProcessor):
    r"""
    :class:`~hf_paddle.LogitsProcessor` that enforces the specified token as the first generated token.
...
class ForcedEOSTokenLogitsProcessor(LogitsProcessor):
    r"""
    :class:`~hf_paddle.LogitsProcessor` that enforces the specified token as the last generated token when
...
class InfNanRemoveLogitsProcessor(LogitsProcessor):
    r"""
    :class:`~hf_paddle.LogitsProcessor` that removes all :obj:`nan` and :obj:`inf` values to avoid the generation
