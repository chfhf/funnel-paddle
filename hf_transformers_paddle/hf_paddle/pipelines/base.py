...
def infer_framework_load_model(
    ...
    **model_kwargs
    ...
    Args:
        model (:obj:`str`, :class:`~hf_paddle.PreTrainedModel` or :class:`~hf_paddle.TFPreTrainedModel`):
            The model to infer the framework from. If :obj:`str`, a checkpoint name. The model to infer the framewrok
            from.
        config (:class:`~hf_paddle.AutoConfig`):
    ...
    if isinstance(model, str):
        ...
        if config.architectures:
            ...
            for architecture in config.architectures:
                hf_paddle_module = importlib.import_module("hf_paddle")
                if look_pt:
                    _class = getattr(hf_paddle_module, architecture, None)
                    if _class is not None:
                        classes.append(_class)
                if look_tf:
                    _class = getattr(hf_paddle_module, f"TF{architecture}", None)
                    if _class is not None:
                        classes.append(_class)
...
class PipelineException(Exception):
    """
    Raised by a :class:`~hf_paddle.Pipeline` when handling __call__.
...
class ArgumentHandler(ABC):
    """
    Base interface for handling arguments for each :class:`~hf_paddle.pipelines.Pipeline`.
    """
...
class PipelineDataFormat:
    ...
    def save(self, data: Union[dict, List[dict]]):
        ...
        :class:`~hf_paddle.pipelines.PipelineDataFormat`.
    ...
    def from_str(
        ...
        """
        Creates an instance of the right subclass of :class:`~hf_paddle.pipelines.PipelineDataFormat` depending on
        :obj:`format`.
        ...
        Returns:
            :class:`~hf_paddle.pipelines.PipelineDataFormat`: The proper data format.
        """
        if format == "json":
...
PIPELINE_INIT_ARGS = r"""
    Arguments:
        model (:obj:`~hf_paddle.PreTrainedModel` or :obj:`~hf_paddle.TFPreTrainedModel`):
            The model that will be used by the pipeline to make predictions. This needs to be a model inheriting from
            :class:`~hf_paddle.PreTrainedModel` for PyTorch and :class:`~hf_paddle.TFPreTrainedModel` for
            TensorFlow.
        tokenizer (:obj:`~hf_paddle.PreTrainedTokenizer`):
            The tokenizer that will be used by the pipeline to encode data for the model. This object inherits from
            :class:`~hf_paddle.PreTrainedTokenizer`.
        modelcard (:obj:`str` or :class:`~hf_paddle.ModelCard`, `optional`):
        ...
        args_parser (:class:`~hf_paddle.pipelines.ArgumentHandler`, `optional`):
...
class Pipeline(_ScikitCompat):
    ...
    Some pipeline, like for instance :class:`~hf_paddle.FeatureExtractionPipeline` (:obj:`'feature-extraction'` )
    ...
    def transform(self, X):
        """
        Scikit / Keras interface to hf_paddle' pipelines. This method will forward to __call__().
        """
        return self(X=X)
