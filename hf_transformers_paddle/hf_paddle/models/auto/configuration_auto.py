...
def _get_class_name(model_class):
    if isinstance(model_class, (list, tuple)):
        return " or ".join([f":class:`~hf_paddle.{c.__name__}`" for c in model_class])
    return f":class:`~hf_paddle.{model_class.__name__}`"
...
def _list_model_options(indent, config_to_class=None, use_model_types=True):
    ...
    if use_model_types:
        if config_to_class is None:
            model_type_to_name = {
                model_type: f":class:`~hf_paddle.{config.__name__}`"
        ...
        lines = [
            f"{indent}- :class:`~hf_paddle.{config_name}` configuration class: {config_to_name[config_name]} ({config_to_model_name[config_name]} model)"
...
class AutoConfig:
    ...
    when created with the :meth:`~hf_paddle.AutoConfig.from_pretrained` class method.
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
            ...
            pretrained_model_name_or_path (:obj:`str` or :obj:`os.PathLike`):
                Can be either:
                    ...
                    - A path to a `directory` containing a configuration file saved using the
                      :meth:`~hf_paddle.PretrainedConfig.save_pretrained` method, or the
                      :meth:`~hf_paddle.PreTrainedModel.save_pretrained` method, e.g., ``./my_model_directory/``.
        ...
        Examples::
            ...
            >>> from hf_paddle import AutoConfig
