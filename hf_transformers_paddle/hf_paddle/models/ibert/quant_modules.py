...
from paddorch.autograd import Function
...
class QuantEmbedding(nn.Module):
    ...
    def forward(self, x, positions=None, incremental_state=None):
        ...
        A= symmetric_linear_quantization_params(self.weight_bit, w_min, w_max, False)
        torch.copy(A, self.weight_scaling_factor )
        A = self.weight_function(
            self.weight, self.weight_bit, self.percentile_mode, self.weight_scaling_factor
        )
        torch.copy(A, self.weight_integer)
...
class QuantAct(nn.Module):
    ...
    def __init__(self, activation_bit, act_range_momentum=0.95, per_channel=False, channel_len=None, quant_mode=False):
        ...
        if not self.per_channel:
            ...
            torch.copy(self.x_min-1e-5,self.x_min)
            torch.copy(self.x_max + 1e-5, self.x_max)
            # self.x_min -= 1e-5
            # self.x_max += 1e-5
        else:
    ...
    def forward(
        ...
        if self.training:
            ...
            if self.x_min.min() > -1.1e-5 and self.x_max.max() < 1.1e-5:
                A = self.x_min + x_min
                torch.copy(A,self.x_min)
                A = self.x_max + x_max
                torch.copy(A, self.x_max)

            # exponential moving average (EMA)
            # use momentum to prevent the quantized values change greatly every iteration
            elif self.act_range_momentum == -1:
                A = torch.min(self.x_min, x_min)
                torch.copy(A, self.x_min)
                A= torch.max(self.x_max, x_max)
                torch.copy(A, self.x_max)
            else:
                A = self.x_min * self.act_range_momentum + x_min * (1 - self.act_range_momentum)
                torch.copy(A, self.x_min)
                A  = self.x_max * self.act_range_momentum + x_max * (1 - self.act_range_momentum)
                torch.copy(A, self.x_max)
        ...
        A= symmetric_linear_quantization_params(
            self.activation_bit, x_min, x_max, per_channel=self.per_channel
        )
        torch.copy(A,self.act_scaling_factor)
...
class QuantLinear(nn.Module):
    ...
    def forward(self, x, prev_act_scaling_factor=None):
        ...
        assert prev_act_scaling_factor is not None  , (
        ...
        w_transform = w.detach()
        if self.per_channel:
            w_min= torch.min(w_transform, dim=1 )
            w_max= torch.max(w_transform, dim=1 )
        else:
            w_min = w_transform.min().expand(1)
            w_max = w_transform.max().expand(1)

        A = symmetric_linear_quantization_params(self.weight_bit, w_min, w_max, self.per_channel)
        torch.copy(A,self.fc_scaling_factor)
        A = self.weight_function(
        ...
        if self.bias is not None:
            A = self.weight_function(self.bias, self.bias_bit, False, bias_scaling_factor)
            torch.copy(A,self.bias_integer)
...
class IntGELU(nn.Module):
    ...
    def int_erf(self, x_int, scaling_factor):
        ...
        abs_int = torch.clamp(torch.abs(x_int),b_int, -b_int)
...
class IntSoftmax(nn.Module):
    ...
    def int_exp(self, x_int, scaling_factor):
        ...
        x_int =torch.clamp( x_int , self.const * x0_int,-self.const * x0_int)
    ...
    def forward(self, x, scaling_factor):
        ...
        x_int_max = x_int.max(dim=-1, keepdim=True)
...
class IntLayerNorm(nn.Module):
    ...
    def set_shift(self, y_int):
        with torch.no_grad():
            y_sq_int = y_int ** 2
            var_int = torch.sum(y_sq_int, dim=2, keepdim=True)
            shift = (torch.log2(torch.sqrt(var_int / 2 ** self.max_bit)).ceil()).max()
            shift_old = self.shift
            torch.copy(max(self.shift,  shift ), self.shift)
    ...
    def forward(self, x, scaling_factor=None):
        ...
        x_int = (x.astype("float32") / scaling_factor)
        mean_int = round_ste().apply(x_int.mean(dim=2, keepdim=True))
        ...
        bias = self.bias.detach() / (self.weight.detach())
...
def symmetric_linear_quantization_params(num_bits, saturation_min, saturation_max, per_channel=False):
    ...
    with torch.no_grad():
        ...
        if per_channel:
            scale = torch.max(torch.stack([saturation_min.abs(), saturation_max.abs()], dim=1), dim=1)
...
class FixedPointMul(Function):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            _B = z_scaling_factor.astype("float64") #(z_scaling_factor.type(torch.float)).type(torch.double)
            new_scale = _A / _B
            new_scale = reshape(new_scale)

            m, e = batch_frexp(new_scale)

            # output = z_int.astype("float64")*m.astype("float64") #.type(torch.double) * m #.type(torch.double)
            # output = torch.round(output / (2.0 ** e.astype("float64")))
            output=z_int.astype("float64")*m.astype("float64") / (2.0 ** e.astype("float64"))
            ...
            if identity is not None:
                ...
                _A = identity_scaling_factor.astype("float64") # .type(torch.double)
                _B = z_scaling_factor.astype("float64") #(z_scaling_factor.type(torch.float)) #.type(torch.double)
                new_scale = _A / _B
                new_scale = reshape(new_scale)

                m1, e1 = batch_frexp(new_scale)
                output1 = wx_int.astype("float64")* m1.astype("float64") #.type(torch.double) * m1.type(torch.double)
                output1 = torch.round(output1 / (2.0 ** e1))

                output = output1 + output

            return torch.clamp(output.astype("float32") , -n - 1, n)
