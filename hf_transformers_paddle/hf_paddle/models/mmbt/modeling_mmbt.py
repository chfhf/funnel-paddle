...
class ModalEmbeddings(nn.Module):
    ...
    def forward(self, input_modal, start_token=None, end_token=None, position_ids=None, token_type_ids=None):
        ...
        if position_ids is None:
            ...
            position_ids = position_ids.unsqueeze(0).expand(input_modal.shape[0], seq_length)

        if token_type_ids is None:
            token_type_ids = torch.zeros(
                (input_modal.shape[0], seq_length), dtype=torch.long, device=input_modal.device
            )
...
MMBT_START_DOCSTRING = r"""
    MMBT model was proposed in `Supervised Multimodal Bihf_paddle for Classifying Images and Text
    ...
    Parameters:
        config (:class:`~hf_paddle.MMBTConfig`): Model configuration class with all the parameters of the model.
...
MMBT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        input_ids (``torch.LongTensor`` of shape ``(batch_size, sequence_length)``):
            ...
            :class:`~hf_paddle.BertTokenizer`. See :meth:`hf_paddle.PreTrainedTokenizer.encode` and
...
class MMBTModel(nn.Module, ModuleUtilsMixin):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            input_txt_shape = input_ids.shape
        elif inputs_embeds is not None:
            input_txt_shape = inputs_embeds.shape[:-1]
        else:
        ...
        input_modal_shape = modal_embeddings.shape[:-1]
