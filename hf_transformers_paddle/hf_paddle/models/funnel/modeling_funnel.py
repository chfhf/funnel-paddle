...
class FunnelAttentionStructure(nn.Module):
    ...
    def token_type_ids_to_mat(self, token_type_ids):
        ...
        cls_mat = torch.logical_or(cls_ids.unsqueeze(2), cls_ids.unsqueeze(1))
        return torch.logical_or(cls_mat , token_type_mat)
    ...
    def get_position_embeds(self, seq_len, dtype, device):
        ...
        if self.config.attention_type == "factorized":
            ...
            freq_seq = torch.arange(0, d_model // 2, 1, dtype=dtype, device=device)
            inv_freq = 1 / (10000 ** (freq_seq / (d_model // 2)))
            # Maximum relative positions for the first input
            rel_pos_id = torch.arange(-seq_len * 2, seq_len * 2, 1, dtype=dtype, device=device)
    ...
    def stride_pool_pos(self, pos_id, block_index):
        ...
        if self.config.separate_cls:
            ...
            cls_pos = pos_id.new_tensor([-(2 ** block_index) + 1]).astype(pos_id.dtype)
    ...
    def stride_pool(self, tensor, axis):
        ...
        if self.config.separate_cls:
            ...
            if axis==1:
                tensor = torch.cat([tensor[:,:1], tensor], dim=axis)
            if axis==2:
                tensor = torch.cat([tensor[:,:,:1], tensor], dim=axis)
            if axis==0:
                tensor = torch.cat([tensor[:1], tensor], dim=axis)
...
class FunnelRelMultiheadAttention(nn.Module):
    ...
    def relative_token_type_attention(self, token_type_mat, q_head, cls_mask=None):
        ...
        token_type_mat = token_type_mat.unsqueeze(1).expand([batch_size, q_head.shape[2], seq_len, context_len])
    ...
    def forward(self, query, key, value, attention_inputs, output_attentions=False):
        ...
        if attention_mask is not None:
            ...
            attn_score = attn_score - INF * (1 - attention_mask.unsqueeze(1).unsqueeze(2).float())
...
class FunnelBaseModel(FunnelPreTrainedModel):
    ...
    def forward(
        ...
        # print("debug:", encoder_outputs[0].max(), encoder_outputs[0].mean())
...
class FunnelForPreTraining(FunnelPreTrainedModel):
    ...
    def forward(
        ...
        Examples::
            ...
            >>> from hf_paddle import FunnelTokenizer, FunnelForPreTraining
            >>> import paddorch as torch
