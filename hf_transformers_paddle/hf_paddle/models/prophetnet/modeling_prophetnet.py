...
import paddorch.utils.checkpoint
from paddorch import Tensor, nn
from paddorch.nn import LayerNorm
...
PROPHETNET_START_DOCSTRING = r"""
    This model inherits from :class:`~hf_paddle.PreTrainedModel`. Check the superclass documentation for the generic
    ...
    Parameters:
        config (:class:`~hf_paddle.ProphetNetConfig`): Model configuration class with all the parameters of the model.
...
PROPHETNET_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
            ...
            Indices can be obtained using :class:`~hf_paddle.ProphetNetTokenizer`. See
            :meth:`hf_paddle.PreTrainedTokenizer.encode` and :meth:`hf_paddle.PreTrainedTokenizer.__call__` for
            details.
...
def softmax(hidden_state, dim, onnx_trace=False):
    if onnx_trace:
        ...
        return nn.functional.softmax(hidden_state, dim=dim )
...
def ngram_attention_bias(sequence_length, ngram, device, dtype):
    ...
    """
    left_block = torch.ones((ngram, sequence_length, sequence_length), device=device, dtype=dtype) * -1e+20
    right_block = left_block.detach().clone()
    # create bias

    for stream_idx in range(ngram):
        # diag_v=paddle.diag(right_block[stream_idx])
        #
        # diag_v=torch.mm(paddle.eye(right_block[stream_idx].shape[0], right_block[stream_idx].shape[1]), paddle.expand_as(diag_v,right_block[stream_idx]))
        # right_block[stream_idx]=right_block[stream_idx]-diag_v
        right_block[stream_idx]=right_block[stream_idx].fill_diagonal(0, wrap=False)
...
def compute_relative_buckets(num_buckets, max_distance, relative_positions, is_bidirectional=False):
    ...
    if is_bidirectional:
        ...
        rel_positions_bucket = (
            rel_positions_bucket
            + (inv_relative_positions<0).int() * num_buckets  #torch.lt(inv_relative_positions, torch.zeros_like(inv_relative_positions)).int() * num_buckets
        )
        ...
        inv_relative_positions = torch.clamp(inv_relative_positions,0,1e+6)  #torch.max(inv_relative_positions, torch.zeros_like(inv_relative_positions))

    max_exact = num_buckets // 2
    is_small = inv_relative_positions< max_exact
    ...
    val_if_large = torch.clamp(val_if_large,  -1e+6,(num_buckets - 1)).int()
    ...
    def forward(
        ...
        if layer_head_mask is not None:
            assert layer_head_mask.shape == (
                self.num_attn_heads,
            ), f"Head mask for a single layer should be of size {(self.num_attn_heads,)}, but is {layer_head_mask.shape}"
        ...
        assert attn_output.shape == (
            ...
            tgt_len,
            self.head_dim,
        ), "`attn_output` should be of shape {batch_size * self.num_attn_heads, tgt_len, self.head_dim}, but is of shape {attn_output.shape}"
...
class ProphetNetNgramSelfAttention(nn.Module):
    ...
    def get_main_relative_pos_embeddings(
        ...
        rel_pos_embeddings = rel_pos_embeddings.reshape(-1, rel_pos_embeddings.shape[-1])  # [B*head*T,Buckets]
...
class ProphetNetEncoder(ProphetNetPreTrainedModel):
    ...
    word_embeddings  (:obj:`torch.nn.Embeddings` of shape :obj:`(config.vocab_size, config.hidden_size)`, `optional`):
        The word embedding parameters. This can be used to initialize :class:`~hf_paddle.ProphetNetEncoder` with
    ...
    def forward(
        ...
        Example::
            ...
            >>> from hf_paddle import ProphetNetTokenizer, ProphetNetEncoder
            >>> import paddorch as torch
        ...
        if attention_mask is not None:
            extended_attention_mask = (
                1.0 - attention_mask.unsqueeze(1).repeat(self.config.num_encoder_attention_heads, 1, 1)
            ) * -10000.0
...
class ProphetNetDecoder(ProphetNetPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> from hf_paddle import ProphetNetTokenizer, ProphetNetDecoder
            >>> import paddorch as torch
        ...
        if encoder_attention_mask is not None:
            extended_encoder_attention_mask = (
                1.0 - encoder_attention_mask.unsqueeze(1).repeat(self.config.num_decoder_attention_heads, 1, 1)
            ) * -10000.0
    ...
    def prepare_attention_mask(self, hidden_states, attention_mask):
        ...
        extended_causal_mask = causal_mask[:seq_length, :seq_length].unsqueeze(0).expand(
    ...
    def prepare_predict_attention_mask(self, hidden_states, attention_mask):
        ...
        extended_predict_causal_mask = predict_causal_mask.unsqueeze(1).expand(
            predict_causal_mask.shape[:1] + (batch_size,) + predict_causal_mask.shape[1:]
        )

        # add usual attention mask
        if attention_mask is not None:
            extended_attention_mask = (1.0 - attention_mask.unsqueeze(0).unsqueeze(2)) * -10000.0
...
class ProphetNetModel(ProphetNetPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> from hf_paddle import ProphetNetTokenizer, ProphetNetModel
        ...
        #         "https://github.com/huggingface/hf_paddle/issues/9804. Please try to use an off-the-shelf"
...
class ProphetNetForConditionalGeneration(ProphetNetPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> from hf_paddle import ProphetNetTokenizer, ProphetNetForConditionalGeneration
    ...
    @staticmethod
    # Copied from hf_paddle.models.bart.modeling_bart.BartForConditionalGeneration._reorder_cache
...
class ProphetNetForCausalLM(ProphetNetPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> from hf_paddle import ProphetNetTokenizer, ProphetNetForCausalLM
            >>> import paddorch as torch
            ...
            >>> from hf_paddle import BertTokenizer, EncoderDecoderModel, ProphetNetTokenizer
            >>> import paddorch as torch
    ...
    @staticmethod
    # Copied from hf_paddle.models.bart.modeling_bart.BartForCausalLM._reorder_cache
    def _reorder_cache(past, beam_idx):
        reordered_past = ()
        for layer_past in past:
...
class ProphetNetDecoderWrapper(ProphetNetPreTrainedModel):
    """
    This is a wrapper class, so that :class:`~hf_paddle.ProphetNetForCausalLM` can correctly be loaded from
