...
class LMOrderedIterator(object):
    ...
    def get_batch(self, i, bptt=None):
        if bptt is None:
            bptt = self.bptt
        seq_len = min(bptt, self.data.shape[0] - 1 - i)
...
class LMShuffledIterator(object):
    ...
    def stream_iterator(self, sent_stream):
        ...
        while True:
            ...
            n_retain = min(data.shape[0], self.ext_len)
            if n_retain > 0:
                data[:n_retain] = data[-n_retain:]
            data.resize_(n_retain + self.bptt, data.shape[1])
