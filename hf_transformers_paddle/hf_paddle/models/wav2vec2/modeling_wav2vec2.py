...
class Wav2Vec2BaseModelOutput(ModelOutput):
    """
    Output type of :class:`~hf_paddle.Wav2Vec2BaseModelOutput`, with potential hidden states and attentions.
...
class Wav2Vec2ForPreTrainingOutput(ModelOutput):
    """
    Output type of :class:`~hf_paddle.Wav2Vec2ForPreTrainingOutput`, with potential hidden states and attentions.
...
# Copied from hf_paddle.models.bart.modeling_bart.BartAttention with Bart->Wav2Vec2
...
WAV_2_VEC_2_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~hf_paddle.Wav2Vec2Config`): Model configuration class with all the parameters of the model.
...
class Wav2Vec2Model(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> from hf_paddle import Wav2Vec2Processor, Wav2Vec2Model
...
class Wav2Vec2ForPreTraining(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> import paddorch as torch
            >>> from hf_paddle import Wav2Vec2FeatureExtractor, Wav2Vec2ForPreTraining
            >>> from hf_paddle.models.wav2vec2.modeling_wav2vec2 import _compute_mask_indices
            >>> from datasets import load_dataset
            >>> import soundfile as sf
        ...
        if self.training:
            ...
            if neg_is_pos.any():
                logits[1:][neg_is_pos] = -1e+20

            # 6. compute contrastive loss \mathbf{L}_m = cross_entropy(logs) =
            # -log(exp(sim(c_t, q_t)/\kappa) / \sum_{\sim{q}} exp(sim(c_t, \sim{q})/\kappa))
            preds = logits.transpose(0, 2).reshape(-1, logits.shape[0])
...
class Wav2Vec2ForCTC(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> import paddorch as torch
            >>> from hf_paddle import Wav2Vec2Processor, Wav2Vec2ForCTC
            >>> from datasets import load_dataset
            >>> import soundfile as sf
