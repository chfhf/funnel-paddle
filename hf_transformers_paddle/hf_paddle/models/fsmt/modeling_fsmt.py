...
# - beam search: stops as soon as num_beams == len(hypos) (whereas hf_paddle
#   is not satisfied there and will continue searching until the next cycles
#   aren't promising something better), comparing BLEU scores - the hf_paddle
...
FSMT_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~hf_paddle.FSMTConfig`): Model configuration class with all the parameters of the model.
...
FSMT_GENERATION_EXAMPLE = r"""
    Translation example::
        ...
        from hf_paddle import FSMTTokenizer, FSMTForConditionalGeneration
...
FSMT_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
            ...
            IIndices can be obtained using :class:`~hf_paddle.FSTMTokenizer`. See
        ...
        decoder_input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, target_sequence_length)`, `optional`):
            ...
            Indices can be obtained using :class:`~hf_paddle.FSMTTokenizer`. See
            :meth:`hf_paddle.PreTrainedTokenizer.encode` and :meth:`hf_paddle.PreTrainedTokenizer.__call__` for
            details.
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:  # don't attend to padding symbols
            ...
            attn_weights = attn_weights.masked_fill(reshaped, -1e+20)
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
...
def fill_with_neg_inf(t):
    ...
    return t.float().fill_(-1e+20).type_as(t)
