...
from paddorch.nn.utils.rnn import pad_sequence
...
class DataCollatorWithPadding:
    ...
    Args:
        tokenizer (:class:`~hf_paddle.PreTrainedTokenizer` or :class:`~hf_paddle.PreTrainedTokenizerFast`):
...
class DataCollatorForSeq2Seq:
    ...
    Args:
        tokenizer (:class:`~hf_paddle.PreTrainedTokenizer` or :class:`~hf_paddle.PreTrainedTokenizerFast`):
            The tokenizer used for encoding the data.
        model (:class:`~hf_paddle.PreTrainedModel`):
...
class DataCollatorForLanguageModeling:
    ...
    .. note::
        ...
        :class:`~hf_paddle.PreTrainedTokenizer` or a :class:`~hf_paddle.PreTrainedTokenizerFast` with the
...
class DataCollatorForWholeWordMask(DataCollatorForLanguageModeling):
    ...
    .. note::
        ...
        :class:`~hf_paddle.BertTokenizer`, specifically that subword tokens are prefixed with `##`. For tokenizers
...
class DataCollatorForPermutationLanguageModeling:
    ...
    def mask_tokens(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        target_mapping = torch.zeros((labels.shape[0], labels.shape[1], labels.shape[1]), dtype=torch.float32)
        ...
        for i in range(labels.shape[0]):
            ...
            target_mapping[i] = torch.eye(labels.shape[1])
        ...
        perm_mask = torch.zeros((labels.shape[0], labels.shape[1], labels.shape[1]), dtype=torch.float32)
        ...
        for i in range(labels.shape[0]):
            ...
            perm_index = torch.arange(labels.shape[1])
            # Split this into two halves, assuming that half the sequence is reused each time
            perm_index = perm_index.reshape((-1, labels.shape[1] // 2)).transpose(0, 1)
            # Permute the two halves such that they do not cross over
            perm_index = perm_index[torch.randperm(labels.shape[1] // 2)]
