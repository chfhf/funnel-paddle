...
from paddorch.utils.data import Dataset, IterableDataset
from paddorch.utils.data.distributed import DistributedSampler
from paddorch.utils.data.sampler import RandomSampler, SequentialSampler
...
class Trainer:
    ...
    Args:
        model (:class:`~hf_paddle.PreTrainedModel` or :obj:`torch.nn.Module`, `optional`):
            The model to train, evaluate or use for predictions. If not provided, a ``model_init`` must be passed.

            .. note::

                :class:`~hf_paddle.Trainer` is optimized to work with the :class:`~hf_paddle.PreTrainedModel`
                provided by the library. You can still use your own models defined as :obj:`torch.nn.Module` as long as
                they work the same way as the 🤗 Transformers models.
        args (:class:`~hf_paddle.TrainingArguments`, `optional`):
            The arguments to tweak for training. Will default to a basic instance of
            :class:`~hf_paddle.TrainingArguments` with the ``output_dir`` set to a directory named `tmp_trainer` in
            the current directory if not provided.
        data_collator (:obj:`DataCollator`, `optional`):
            The function to use to form a batch from a list of elements of :obj:`train_dataset` or :obj:`eval_dataset`.
            Will default to :func:`~hf_paddle.default_data_collator` if no ``tokenizer`` is provided, an instance of
            :func:`~hf_paddle.DataCollatorWithPadding` otherwise.
        ...
        model_init (:obj:`Callable[[], PreTrainedModel]`, `optional`):
            ...
            :meth:`~hf_paddle.Trainer.train` will start from a new instance of the model as given by this function.
        ...
        compute_metrics (:obj:`Callable[[EvalPrediction], Dict]`, `optional`):
            ...
            :class:`~hf_paddle.EvalPrediction` and return a dictionary string to metric values.
        callbacks (List of :obj:`~hf_paddle.TrainerCallback`, `optional`):
        ...
        optimizers (:obj:`Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler.LambdaLR`, `optional`): A tuple
            ...
            :class:`~hf_paddle.AdamW` on your model and a scheduler given by
            :func:`~hf_paddle.get_linear_schedule_with_warmup` controlled by :obj:`args`.

    Important attributes:

        - **model** -- Always points to the core model. If using a hf_paddle model, it will be a
          :class:`~hf_paddle.PreTrainedModel` subclass.
    ...
    def _hp_search_setup(self, trial: Union["optuna.Trial", Dict[str, Any]]):
        ...
        if self.args.deepspeed:
            ...
            from hf_paddle.deepspeed import HfDeepSpeedConfig
    ...
    def _wrap_model(self, model, training=True):
        ...
        if self.sharded_ddp is not None:
            ...
            if self.args.ddp_find_unused_parameters is not None:
                ...
                # https://github.com/huggingface/hf_paddle/pull/4659#issuecomment-643356021
        ...
        **kwargs,
        ...
        Args:
            resume_from_checkpoint (:obj:`str` or :obj:`bool`, `optional`):
                ...
                :class:`~hf_paddle.Trainer`. If a :obj:`bool` and equals `True`, load the last checkpoint in
                `args.output_dir` as saved by a previous instance of :class:`~hf_paddle.Trainer`. If present,
        ...
        if resume_from_checkpoint is not None:
            ...
            if os.path.isfile(os.path.join(resume_from_checkpoint, CONFIG_NAME)):
                ...
                checkpoint_version = config.hf_paddle_version
    ...
    def hyperparameter_search(
        ...
        **kwargs,
        ...
        .. warning::
            ...
            :class:`~hf_paddle.Trainer`: we need to reinitialize the model at each new run. This is incompatible
            with the ``optimizers`` argument, so you need to subclass :class:`~hf_paddle.Trainer` and override the
            method :meth:`~hf_paddle.Trainer.create_optimizer_and_scheduler` for custom optimizer/scheduler.

        Args:
            hp_space (:obj:`Callable[["optuna.Trial"], Dict[str, float]]`, `optional`):
                A function that defines the hyperparameter search space. Will default to
                :func:`~hf_paddle.trainer_utils.default_hp_space_optuna` or
                :func:`~hf_paddle.trainer_utils.default_hp_space_ray` depending on your backend.
            compute_objective (:obj:`Callable[[Dict[str, float]], float]`, `optional`):
                A function computing the objective to minimize or maximize from the metrics returned by the
                :obj:`evaluate` method. Will default to :func:`~hf_paddle.trainer_utils.default_compute_objective`.
            ...
            direction(:obj:`str`, `optional`, defaults to :obj:`"minimize"`):
                ...
                several metrics.
            backend(:obj:`str` or :class:`~hf_paddle.training_utils.HPSearchBackend`, `optional`):
        ...
        Returns:
            :class:`hf_paddle.trainer_utils.BestRun`: All the information about the best run.
        """
        if backend is None:
    ...
    def floating_point_ops(self, inputs: Dict[str, Union[torch.Tensor, Any]]):
        """
        For models that inherit from :class:`~hf_paddle.PreTrainedModel`, uses that method to compute the number of
    ...
    def push_to_hub(self, commit_message: Optional[str] = "add model", **kwargs) -> str:
        ...
        Parameters:
            ...
            kwargs:
                Additional keyword arguments passed along to :meth:`~hf_paddle.Trainer.create_model_card`.
