...
from hf_paddle import logging as hf_paddle_logging
...
class CaptureLogger:
    ...
    Example::
        ...
        >>> from hf_paddle import logging
        >>> from hf_paddle.testing_utils import CaptureLogger

        >>> msg = "Testing 1, 2, 3"
        >>> logging.set_verbosity_info()
        >>> logger = logging.get_logger("hf_paddle.models.bart.tokenization_bart")
...
def LoggingLevel(level):
    """
    This is a context manager to temporarily change hf_paddle modules logging level to the desired value and have it
    ...
    """
    orig_level = hf_paddle_logging.get_verbosity()
    try:
        hf_paddle_logging.set_verbosity(level)
        yield
    finally:
        hf_paddle_logging.set_verbosity(orig_level)
...
class TestCasePlus(unittest.TestCase):
    ...
    * ``pathlib`` objects (all fully resolved):
       ...
       - ``src_dir`` - the directory of ``src`` (i.e. where the ``hf_paddle`` sub-dir resides)
...
def nested_simplify(obj, decimals=3):
    ...
    from hf_paddle.tokenization_utils import BatchEncoding
