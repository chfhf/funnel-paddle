...
class TFTrainer:
    ...
    Args:
        model (:class:`~hf_paddle.TFPreTrainedModel`):
            The model to train, evaluate or use for predictions.
        args (:class:`~hf_paddle.TFTrainingArguments`):
        ...
        optimizers (:obj:`Tuple[tf.keras.optimizers.Optimizer, tf.keras.optimizers.schedules.LearningRateSchedule]`, `optional`):
            ...
            :class:`~hf_paddle.AdamWeightDecay`. The scheduler will default to an instance of
            :class:`tf.keras.optimizers.schedules.PolynomialDecay` if :obj:`args.num_warmup_steps` is 0 else an
            instance of :class:`~hf_paddle.WarmUp`.
    """
    ...
    def __init__(
        ...
        warnings.warn(
            ...
            "examples at https://github.com/huggingface/hf_paddle/tree/master/examples/tensorflow",
            FutureWarning,
        )
    ...
    def setup_comet(self):
        ...
        if experiment is not None:
            experiment._set_model_graph(self.model, framework="hf_paddle")
            experiment._log_parameters(self.args, prefix="args/", framework="hf_paddle")
            experiment._log_parameters(self.model.config, prefix="config/", framework="hf_paddle")
    ...
    def prediction_loop(
        ...
        """
        Prediction/evaluation loop, shared by :func:`~hf_paddle.TFTrainer.evaluate` and
        :func:`~hf_paddle.TFTrainer.predict`.
        ...
        if is_comet_available():
            ...
            if experiment is not None:
                experiment._log_metrics(
                    logs, step=self.global_step, epoch=self.epoch_logging, framework="hf_paddle"
                )
