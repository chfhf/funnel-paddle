...
from paddlenlp.transformers import FunnelForSequenceClassification, FunnelTokenizerFast
...
MODEL_CLASSES = {
    ...
    "funnel":(FunnelForSequenceClassification, FunnelTokenizerFast),
}
...
@paddle.no_grad()
def evaluate(model, loss_fct, metric, data_loader,max_eval_sample=10000000000):
    if not freeze_basemodel:
        model.eval()
    metric.reset()
    count=0
...
def convert_example(example, tokenizer, label_list, max_seq_length=512, is_test=False):
    ...
    if (int(is_test) + len(example)) == 2:
        example = tokenizer(example["sentence"],  return_offsets_mapping=True,max_length=max_seq_length,return_overflowing_tokens=True,return_token_type_ids=True)
    else:
        example = tokenizer(
            example["sentence1"],
...
def do_train(args):
    ...
    set_seed(args)
    log_writer=LogWriter(logdir=f"./log/{args.task_name}")
    ...
    train_batch_sampler = paddle.io.DistributedBatchSampler(
        train_ds, batch_size=args.batch_size, shuffle=not small_train
    )
    ...
    if args.scheduler_type == "linear":
        lr_scheduler = LinearDecayWithWarmup(
            args.learning_rate, num_training_steps, warmup
        )
    ...
    for epoch in range(args.num_train_epochs):
        for step, batch in enumerate(train_data_loader):
            ...
            input_ids, segment_ids, labels = batch
            if freeze_basemodel:
                outputs=model.funnel(input_ids, token_type_ids=segment_ids)
                last_hidden_state = outputs[0]
                pooled_output = last_hidden_state[:, 0]
                pooled_output.stop_gradient=True
                logits=model.classifier(pooled_output)
            else:
                logits = model(input_ids, token_type_ids=segment_ids)
            loss = loss_fct(logits, labels)
            loss.backward()
            log_writer.add_scalar("train/loss",value=loss,step=global_step)
            optimizer.step()
            lr_scheduler.step()
            optimizer.clear_grad()
            if step>args.logging_steps-1 and small_train:
                print(
            ...
            if global_step % args.save_steps == 0 or global_step == num_training_steps:
                ...
                if args.task_name == "mnli":
                    print("=" * 100)
                    evaluate(model, loss_fct, metric, dev_data_loader_matched,max_eval_sample=len(dev_ds))
                    evaluate(model, loss_fct, metric, dev_data_loader_mismatched,max_eval_sample=len(dev_ds))
                    print("eval done total : %s s" % (time.time() - tic_eval))
                else:
                    print("=" * 100)
                    out_eval=evaluate(model, loss_fct, metric, dev_data_loader,max_eval_sample=len(dev_ds) )
                    log_writer.add_scalar(tag="eval/acc", step=global_step, value=out_eval )
                    print("eval done total : %s s" % (time.time() - tic_eval))
                if out_eval>best_acc:
                    best_acc=out_eval
                ...
                if paddle.distributed.get_rank() == 0:
                    ...
                    old_dir = os.path.join(
                        args.output_dir,
                        "%s_ft_model_%d" % (args.task_name, global_step-args.save_steps),
                    )
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    # Need better way to get inner model of DataParall  el
                    model_to_save = (
                        model._layers
                        if isinstance(model, paddle.DataParallel)
                        else model
    ...
    output_json = os.path.join(
        args.output_dir,
        "final_full_eval.json"
    )
    save_json(out_eval,output_json)
