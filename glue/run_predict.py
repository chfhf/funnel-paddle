...
### change other model in this line
from paddlenlp.transformers import DebertaForSequenceClassification as PaddleForSequenceClassification, DebertaTokenizerFast as PaddleTokenizer
from run_glue import convert_example,evaluate
...
def predict2file(args):
    if args.task_name == "mnli":
        ...
        id2label = dict(
            zip(range(len(test_ds_matched.label_list)), test_ds_matched.label_list)
        )
    else:
    ...
    model = PaddleForSequenceClassification.from_pretrained(args.ckpt_path + "/model_state.pdparams", config=args.ckpt_path+"/model_config.json")
    model.eval()
    tokenizer = PaddleTokenizer.from_pretrained(args.ckpt_path)
